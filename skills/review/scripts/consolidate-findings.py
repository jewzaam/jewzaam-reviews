...
"""Consolidate per-agent JSON findings into a stage directory of per-finding files.
...
Output is a stage directory:
  - ``_envelope.json`` with project metadata, decomposition, and issues
  - one ``<content_hash>.json`` per merged finding
...
def _build_issues(warnings: list[str]) -> list[dict]:
    """Build issue objects from skip warnings. Each warning produces a
    `schema_rejected_input` issue for the stage envelope's `issues` array.
    """
    return [
        {
            "severity": "warning",
...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument(
        ...
        help="stage directory to write _envelope.json + per-finding files",
    )
    parser.add_argument(
    ...
    if not args.output_dir.is_dir():
        print(
            f"error: --output-dir is not a directory: {args.output_dir}",
            file=sys.stderr,
        )
        return 2

    agent_schema = SCHEMAS_DIR / "agent-output.schema.json"
    merged_finding_schema = SCHEMAS_DIR / "merged-finding.schema.json"
    stage_envelope_schema = SCHEMAS_DIR / "stage-envelope.schema.json"
    for sp in (agent_schema, merged_finding_schema, stage_envelope_schema):
        if not sp.exists():
            print(f"error: required schema not found: {sp}", file=sys.stderr)
            return 2
    ...
    if envelope_errors:
        print("error: stage envelope failed schema validation:", file=sys.stderr)
        for e in envelope_errors:
            print(f"  {e}", file=sys.stderr)
        return 1

    for f in findings:
        finding_errors = _validate(f, merged_finding_schema)
        if finding_errors:
            print(
                f"error: finding {f.get('content_hash', '?')} failed schema validation:",
                file=sys.stderr,
            )
    ...
    write_stage_dir(args.output_dir, envelope, findings)

    print(
        f"OK: consolidated {len(valid_agents)} agent output(s) into "
        f"{len(findings)} finding(s) at {args.output_dir}"
    )
    return 0
