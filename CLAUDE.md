...
3. **Every JSON carries a top-level `issues[]` array** (may be empty) — the uniform place to capture meta-issues from the run (permission denials in sub-agents, sub-agent failures, tool unavailability, schema rejection of partial inputs). Findings describe the user's code; issues describe the skill's own operation. **Two of its `kind` values are not failures**: `finding_removed` (diff-scope filter, validator removal) and `finding_truncated` (title clipped to the schema bound) record a deliberate decision for audit. Renderers split them out under "Pipeline Decisions" — a clean run listing a working filter as a **warning** beside a dead sub-agent teaches the reader to skim the one section that exists to be read.
4. **The orchestrated review also carries `run_report`** — `{status, steps[]}` where each step is `{step, status, detail}` and status is `ok|degraded|skipped|failed`. `issues[]` says what went wrong; `run_report.steps[]` says what ran at all, which is the only record of a stage that lost work without raising (a validator batch that never dispatched, a selector that fell back). It is rendered as a `## Run Report` table above the findings in the main markdown, alongside the issue list — the point is that reading the findings file never requires reconstructing the run from logs.
...
.tmp-review/
  ...
  costs.json         # measured per-stage/model cost ledger (also carries `steps`)
  run-report.json    # per-step outcome; written before render and on fatal exit
```
...
- `backend.py` — **the agent seam**. `run_agent(prompt, *, schema, model, allowed_tools, cwd, effort, timeout_s, max_budget_usd, session_id, resume_session_id, persist_session, debug_file, harness) -> AgentResult` shells out to headless `claude -p --output-format json --json-schema ...` or `codex exec --json --output-schema ...`. The schema is passed through byte-for-byte on both paths; see the dual-harness section below. Every model invocation goes through this one function; a future backend (e.g. codex) reimplements only this signature. `AgentResult` carries `output` (validated `structured_output`), backend-reported `cost_usd` (`total_cost_usd` when available), `token_usage`, `permission_denials`, `error`, `error_category` (`timeout | spawn | protocol | agent | budget | schema`), and `session_id`. Child env is scrubbed of `CLAUDE*` vars so a nested session cannot leak in. `--bare` is never used (it refuses OAuth).
...
- **A constraint the agent schema cannot carry is enforced at the first deterministic stage.** Finding `title` has `maxLength: 120` in `merged-finding.schema.json` and the shared findings schema, but OpenAI strict outputs reject `maxLength`, so `agent-output` cannot state it. `consolidate-findings.MAX_TITLE_LENGTH` clips it before `content_hash` is taken (clipping afterwards would key the hash to a string no stage holds) and records a `finding_truncated` issue; `simple_mode.collect_findings` does the same for its path. Removing the bound from the agent schema without adding this cost one run a hard abort two stages later — the general trap is that dropping a constraint at the boundary that could prevent a violation leaves it at the boundaries that can only be fatal.
- **Codex reports tokens but never cost.** `total_cost_usd` is hardcoded `0.0` — "unavailable", not "free" — so normalized tokens are the only cross-harness comparison a Codex run has. `_codex_usage` reads the last `turn.completed` and translates the vocabulary: `cached_input_tokens` is Anthropic's `cache_read_input_tokens`, `reasoning_output_tokens` is billed and weighted as output, and there is no counterpart to Anthropic's cache *writes*. The key is omitted, not zero-filled, when a turn reports nothing.
- **Several `run_agent` parameters are Claude-only, and the Codex path says so rather than pretending.** `max_budget_usd`, `tools`/`allowed_tools`, `session_id`/`persist_session`/`resume_session_id` have no Codex equivalent and are ignored there. The consequences are real and are NOT what the bullets below describe: under Codex `timeout_s` is the only runaway control (no spend cap exists, and no cost is reported to cap against), lens agents are bounded by `--sandbox` alone rather than by a tool allowlist — `danger-full-access` inside an OpenShell sandbox, which is *wider* than the Claude boundary, not narrower — and a budget-stop harvest cannot happen. `model` and `effort` ARE honoured, by resolution rather than pass-through (below). Closing the tool-boundary and spend-cap gaps is open work; the bullets below describe the Claude path.
- **Codex runs one model; the roster names Anthropic tiers.** `lenses.py` assigns `sonnet` to six lenses and `haiku` to two to split cheap work from expensive. Codex has no tier equivalent, so `backend.codex_model()` resolves any Anthropic tier to `$REVIEW_ORCHESTRATOR_CODEX_MODEL` (default `gpt-5.6-luna`); an explicit non-tier model passes through, and an empty value sends no `--model` at all and ledgers `codex:default`. Reasoning effort is `$REVIEW_ORCHESTRATOR_CODEX_EFFORT` (default `high`) sent as `-c model_reasoning_effort=`, not a flag — one setting for every agent, since the model is not tiered either. `AgentResult.model_used` carries what actually ran and is what the ledger records: the previous code dropped the tier silently, so a Codex run reported `review/sonnet` and `review/haiku` rows naming two models that never executed, with a cost split that never happened.
- **Codex has no `--debug-file`; its JSONL stream is the live log.** `backend` points the child's stdout straight at `.tmp-review/agent-logs/<label>.log` and reads it back for parsing, so the file answers "what is it doing right now" mid-run the way the Claude debug log does, and one file serves both purposes.
...
Failed lens agents never block the run; a failed selector triggers the all-lenses fallback; a failed validator batch means its findings pass through unchanged (existing `apply-verdicts.py` semantics). Each of those is a `degraded` row in the run report rather than a silent continuation — `failed` is reserved for a step that ends the run, which is why losing every validator batch is `degraded` while losing every lens agent is `failed`.

**Agent-facing schemas are sent to both harnesses unmodified.** Nothing rewrites a schema at dispatch time and nothing rewrites the agent's result. That is the invariant the whole area is built around: when an adapter edits the schema on the way out, the contract under review is not the contract sent, and a keyword it quietly drops weakens the agent's instructions with nothing reporting the gap.

Enforcement is `orchestrator/tests/test_backend.py::TestDualHarnessSchemas`, which checks every agent-facing schema against the rule set below and separately asserts the bytes sent to each CLI equal the bytes loaded. The rules and the schema list live **in the test, not in `backend.py`** — deliberately, since no production code reads them: the point of this area is that nothing at runtime inspects or rewrites a schema on the way out. A schema that cannot satisfy both harnesses is a diff, not a runtime transform.

The intersection the schemas must satisfy:

| Rule | Which harness demands it |
|---|---|
| No `allOf`/`anyOf`/`oneOf` at the **schema root** | Anthropic (400) |
| `anyOf` is the **only** composition keyword anywhere | OpenAI strict (400 `invalid_json_schema`) |
| Every property listed in `required`, recursively | OpenAI strict |
| `additionalProperties: false` on every object | OpenAI strict |
| No `minLength`/`maxLength`/`minProperties`/`unevaluated*`/`dependent*` — use `pattern` | OpenAI strict |
| Every `pattern` anchored `^...$` and reading-independent | neither — see below |

Anthropic is the looser of the two everywhere except the root rule, so in practice the schemas are written to OpenAI's strict subset and checked against both.

Two consequences worth knowing before editing a schema under `skills/review/schemas/`:

- **A presence rule has to be a union, not a conditional.** Not only because `if`/`then` is unsupported: once every property is `required`, `required: ["new_dimensions"]` is trivially true and `not: {required: [...]}` is unsatisfiable, so the conditional form is *meaningless* rather than merely rejected. `validation-output.schema.json` and `validation-output-simple.schema.json` therefore define `verdicts.items` as an `anyOf` over three complete variants, each pinning `action` with `const` and carrying only its own fields — so a `confirm` verdict has no `remove_reason` property to fill in. Verified exhaustively equivalent to the `allOf`/`if`/`then` form it replaced, and verified accepted by the live Anthropic API.
- **`minLength: 1` is written `pattern: "^[\s\S]+$"`, and the exact form matters.** JSON Schema defines `pattern` as a *search*, but a grammar compiler doing constrained decoding may *full-match* it, and a pattern that means different things under the two readings fails only on the provider side — spec-compliant local validation would never catch it. `^[\s\S]+$` reads identically either way and is exactly equivalent to `minLength: 1`. A bare `\S` is the trap: as a search it means "contains non-whitespace" (correct), full-matched it means "exactly one non-whitespace character" — a silent one-character cap on every justification field. `dual_harness_violations` rejects any unanchored pattern, and a test additionally compares both readings on real values, because `^...$` is only a proxy for the property that actually matters. Resist strengthening a constraint while porting it: an earlier `^\s*\S[\s\S]*$` here also rejected whitespace-only strings, which `minLength: 1` never did, and that gratuitous `\S` was the whole reason anchoring became load-bearing.
- **"Unset" is `null`, not an absent key, everywhere in the pipeline.** An optional field is declared `anyOf: [<type>, {"type": "null"}]` and still listed in `required`. Nothing strips those nulls — that is deliberate, because a stripping step would mean intermediate files no longer validate against the schema their agent was given, and `consolidate-findings.py`, `apply-verdicts.py`, `batch-findings.py` and `validate-findings.py` all validate what they read. Consumers read null as absent instead: `scripts/envelope.primary_location` and `diff-scope-filter.py` use `(loc.get("role") or "primary")`, `apply-verdicts.py` skips null `new_dimensions` members, `simple_mode.apply_verdicts` skips null `new_severity`/`new_confidence`. The lens, selector and validator prompts all state the convention. `lenses.EMPTY_SCOPE` is the all-null scope the orchestrator hands out by default.

The incident behind all of this: `validation-output.schema.json` used to carry a nested `allOf` of `if`/`then` conditionals. Codex rejected every validator request before the agent ran, and `apply-verdicts.py` passes a batch with no verdict file straight through — so a run shipped its findings with nothing having challenged them, and said nothing about it. An adapter that guards only ROOT-level composition (what Anthropic rejects) passes that schema through untouched.
