...
- `backend.py` — **the agent seam**. `run_agent(prompt, *, schema, model, allowed_tools, cwd, effort, timeout_s) -> AgentResult` shells out to headless `claude -p --output-format json --json-schema ...`. Every model invocation goes through this one function; a future backend (e.g. codex) reimplements only this signature. `AgentResult` carries `output` (validated `structured_output`), the **measured** `cost_usd` (`total_cost_usd` from the CLI result), `permission_denials`, `error`, and `error_category`. Child env is scrubbed of `CLAUDE*` vars so a nested session cannot leak in. `--bare` is never used (it refuses OAuth).
- Telemetry: children keep inherited `OTEL_*` and the two `CLAUDE_CODE_*TELEMETRY*` toggles; when the inherited env lacks `OTEL_EXPORTER_OTLP_ENDPOINT`, telemetry keys are filled (never overridden) from `$REVIEW_ORCHESTRATOR_OTEL_ENV`, `/sandbox/.env`, or `~/.env` — and an endpoint implies exporter defaults (`otlp`). Each child reports under its own `session_id` (recorded in `costs.json` entries and the agent trace) and carries run-correlation resource attributes — `review.run_id` (also in `costs.json` and the run summary), `review.stage`, `review.agent`, `review.scoring` — so one PromQL/LogQL filter groups a whole review run (labels arrive as `review_run_id` etc.).
