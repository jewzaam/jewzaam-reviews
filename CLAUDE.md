...
- `backend.py` — **the agent seam**. `run_agent(prompt, *, schema, model, allowed_tools, cwd, effort, timeout_s, max_budget_usd, session_id, resume_session_id, persist_session, debug_file) -> AgentResult` shells out to headless `claude -p --output-format json --json-schema ...`. Every model invocation goes through this one function; a future backend (e.g. codex) reimplements only this signature. `AgentResult` carries `output` (validated `structured_output`), the **measured** `cost_usd` (`total_cost_usd` from the CLI result), `permission_denials`, `error`, `error_category` (`timeout | spawn | protocol | agent | budget | schema`), and `session_id`. Child env is scrubbed of `CLAUDE*` vars so a nested session cannot leak in. `--bare` is never used (it refuses OAuth).
